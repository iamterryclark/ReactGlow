...
joints(_joints) 
{
	...
	gestureOnFont.load("fonts/OpenSans-Light.ttf", 5);
	poseDistFont.load("fonts/OpenSans-Bold.ttf", 3);
...
{
	//Constantly checks on which poses being actiavted
	...
	if (poseDist1 < euclidThreshold) {
		...
		pose1Timer = pose2Timer = pose3Timer = pose4Timer = pose5Timer = 0;
